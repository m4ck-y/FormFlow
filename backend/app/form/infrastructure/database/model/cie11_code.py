...
form_cie11_codes = Table(
    SchemaForm.TBL_FORM_CIE11_CODES.name,
    ...
    Column("id_cie11_code", Integer, ForeignKey(f"{SchemaForm.TBL_CIE11_CODE.identifier}.id"), primary_key=True),
    schema=SchemaForm.TBL_FORM_CIE11_CODES.schema
)
...
class ModelCIE11Code(BaseModel): #TODO: table, updatated_by, deleted_by, etc, se puede hacer una clase que herede de table? y tendria atributos creted_at, by?, etc, ...
    ...
    __table_args__ = {"schema": SchemaForm.TBL_CIE11_CODE.schema}
    ...
    list_forms = relationship(
        "ModelForm",
        secondary=form_cie11_codes,
        back_populates="list_cie11_codes"
    )